...
import {
  DISTRICTS, CATEGORIES, formatPrice,
  ...
  return (
    <a href={`/listings/${_id}`} className="card flex flex-col sm:flex-row group">
      <div className="sm:w-48 lg:w-56 shrink-0 aspect-[4/3] sm:aspect-auto overflow-hidden bg-gray-100">
        <img
          src={images?.[0] ? getCardImageUrl(images[0]) : `https://placehold.co/300x200/f97316/white?text=${category}`}
          alt={title}
          loading="lazy"
          decoding="async"
      ...
      <div className="p-4 flex flex-col justify-between flex-1">
        <div>
          <div className="flex items-start justify-between gap-2 mb-2">
            <h3 className="font-display font-bold text-gray-900 text-base group-hover:text-brand transition-colors leading-snug">
              {truncate(title, 70)}
            </h3>
    ...
    <div className="page-wrapper">
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm">
          {total > 0 ? `${total} properties found` : 'Search properties across Manipur'}
        </p>
      </div>
                ...
                {['', ...CATEGORIES].map((c) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded-xl text-sm transition-colors ${
                      category === c ? 'bg-brand text-white font-semibold' : 'hover:bg-gray-100 text-gray-700'
                    }`}
                  >
                    {c || 'All Categories'}
                  </button>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`p-2 rounded-lg transition-colors ${
                  view === 'grid' ? 'bg-brand text-white' : 'bg-white border border-gray-200 text-gray-500 hover:border-brand'
                }`}
              >
                <Grid3X3 size={16} />
              </button>
              <button
                ...
                className={`p-2 rounded-lg transition-colors ${
                  view === 'list' ? 'bg-brand text-white' : 'bg-white border border-gray-200 text-gray-500 hover:border-brand'
                }`}
              >
                <List size={16} />
              </button>
            ...
            <div className={
              ...
              {listings.map((listing) =>
                view === 'grid'
                  ...
                  : <ListCardRow key={listing._id} listing={listing} />
              )}
            </div>
          )}

          <Pagination currentPage={currentPage} pages={pages} onPage={goPage} />
        </div>
      </div>
    </div>
  );
