...
// ── 360° Video player (CSS perspective drag) ──────────────
  ...
  return (
    ...
    >
      ...
      <p className="absolute bottom-4 left-1/2 -translate-x-1/2
                    bg-black/50 text-white text-xs px-3 py-1.5 rounded-full
...
// ── 360° Image viewer (Photo Sphere Viewer) ───────────────
function Image360({ url }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Dynamic import — Vite bundles this at build time, no CDN needed
        const { Viewer } = await import('@photo-sphere-viewer/core');

        // Import the PSV stylesheet once
        await import('@photo-sphere-viewer/core/index.css').catch(() => {
          // Fallback: inject minimal CSS if the import fails
        ...
        viewer = new Viewer({
          ...
          touchmoveTwoFingers: false, // allow single-finger drag on mobile
        ...
        console.error('[VRViewer] Photo Sphere Viewer error:', err);
        setStatus('error');
      }
    })();
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {status === 'error' && (
        <div className="absolute inset-0 z-10 flex flex-col items-center
                        justify-center bg-gray-950 px-6 text-center">
          <div className="text-4xl mb-4">🥽</div>
          <p className="text-white font-semibold mb-1">Could not load the panorama</p>
          <p className="text-white/50 text-sm mb-4">
            The image may still be processing on Cloudinary.
          </p>
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-gray-950">
      ...
      <div className="flex items-center justify-between px-4 py-3
                      bg-black/80 backdrop-blur-sm shrink-0 border-b border-white/5">
        <div className="flex items-center gap-3">
          {/* VR goggles icon */}
          <div className="w-8 h-8 bg-brand/10 border border-brand/30 rounded-lg
      ...
      {/* ── Bottom hints ─────────────────────── */}
      <div className="bg-black/70 backdrop-blur-sm px-4 py-2 shrink-0
