...
export default function Navbar() {
  ...
  const [settingsOpen, setSettingsOpen] = useState(false); // desktop dropdown
  ...
  useEffect(() => {
    const handler = (e) => { if (dropdownRef.current && !dropdownRef.current.contains(e.target)) setSettingsOpen(false); };
  ...
  // User avatar pill + settings dropdown
  const UserMenu = () => (
    <div className="relative" ref={dropdownRef}>
      ...
      {/* Dropdown */}
      {settingsOpen && (
        <div className="absolute right-0 top-full mt-2 w-52 bg-white rounded-2xl shadow-lg
        ...
        <div className="md:hidden bg-white border-t border-gray-100 px-4 py-4 space-y-0.5">
          {user && (
            <div className="flex items-center gap-3 px-3 py-3 rounded-xl mb-3 bg-gray-50 border border-gray-200">
              {user.avatar
                ? <img src={user.avatar} alt={user.name} className="w-9 h-9 rounded-full shrink-0" />
            ...
            {user ? (
              <button onClick={handleLogout}
                className="w-full flex items-center justify-center gap-2 py-3 rounded-xl
                           text-red-500 font-semibold text-sm border border-red-100
